...
func TestFilterBuilder_MergeWithOr(t *testing.T) {
	// Test convenience method for OR merge
	filter1 := NewFilter().WithWhere(NewWhere().Eq("status", "active"))
	filter2 := NewFilter().WithWhere(NewWhere().Eq("status", "pending"))

	merged := filter1.MergeWithOr(filter2)
	result, err := merged.Build()
	assert.NoError(t, err)
...
func TestFilterBuilder_MergeWithConfig(t *testing.T) {
	// Test alias method for better readability
	filter1 := NewFilter().WithWhere(NewWhere().Eq("status", "active"))
	filter2 := NewFilter().WithWhere(NewWhere().Eq("type", "user"))

	config := &MergeConfig{
		WhereOperator:       "and",
		AllowFieldConflicts: true,
	}
	merged := filter1.MergeWithConfig(filter2, config)
	result, err := merged.Build()
	assert.NoError(t, err)
